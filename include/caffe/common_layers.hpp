 ...
 */
template <typename Dtype>
class GBNLayer : public Layer<Dtype> {
 public:
  explicit GBNLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "GBN"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
 ...
 protected:
  ...
  Dtype allstatistics_mean;
  Dtype allstatistics_std;
  ...
  Blob<Dtype> global_sum_multiplier_;
};
/**
 * @brief Batch normalization the input blob along the channel axis while
 *        averaging over the spatial axes.
