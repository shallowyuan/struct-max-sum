...
static void fromDatum(MEX_ARGS){
	  InputArguments input(nrhs, prhs, 1);
	  OutputArguments output(nlhs, plhs, 2);
	  caffe::Datum datum;
	  mxCHECK(datum.ParseFromString(input.get<string>(0)),
...
static void toEncodedDatum(MEX_ARGS){
...
static void EncodeDatum(MEX_ARGS){
  InputArguments input(nrhs, prhs, 2);
  OutputArguments output(nlhs, plhs, 2);
  caffe::Datum datumx,datumy;
  mxCHECK(datumx.ParseFromString(input.get<string>(0)),
           "Failed to parse datum.");
  datumy=datumx;
  const string& data = datumx.data();
  const bool has_uint8 = data.size() > 0;
  const int datum_channels = datumx.channels();
  const int datum_height = datumx.height();
  const int datum_width = datumx.width();

  cv::Mat M,Mx,My; 
  if (datum_channels==1)
    M=cv::Mat(datum_height, datum_width, has_uint8?CV_8UC1:CV_32FC1);
  if (datum_channels==2)
    M=cv::Mat(datum_height, datum_width, has_uint8?CV_8UC2:CV_32FC2);
  else
    M=cv::Mat(datum_height, datum_width, has_uint8?CV_8UC3:CV_32FC3);
  ...
  for (int c = 0; c < datum_channels; ++c) {
    for (int h = 0; h < datum_height; ++h) {
      for (int w = 0; w < datum_width; ++w) {
        ...
        if (has_uint8) {
          if (datum_channels>=3)
            M.at<cv::Vec3b>(h, w)[c] = static_cast<uint8_t>(data[data_index]);
          else if (datum_channels==2)
            M.at<cv::Vec2b>(h, w)[c] = static_cast<uint8_t>(data[data_index]);
          else
            M.at<uchar>(h, w) = static_cast<uint8_t>(data[data_index]);
        }else{
          if (datum_channels>=3)
            M.at<cv::Vec3f>(h, w)[c] = datumx.float_data(data_index);
          else if (datum_channels==2)
            M.at<cv::Vec2f>(h, w)[c] = datumx.float_data(data_index);
          else
            M.at<float>(h, w) = datumx.float_data(data_index);
        }
      }
    }
  }
  if (datum_channels==2){
    std::vector<uchar> bufx,bufy;
    cv::Mat Mx=cv::Mat(datum_height, datum_width, has_uint8?CV_8UC1:CV_32FC1);
    cv::Mat My=cv::Mat(datum_height, datum_width, has_uint8?CV_8UC1:CV_32FC1);
    int from_to[] = { 0,0, 1,1};
    cv::Mat out[] = { Mx, My };
    mixChannels( &M, 1, out, 2, from_to, 2 );
    cv::imencode(".jpg", Mx, bufx);
    datumx.set_channels(1);
    datumx.clear_data();
    datumx.clear_float_data();
    CHECK(Mx.depth() == CV_8U) << "Image data type must be unsigned byte";
    datumx.set_data(std::string(reinterpret_cast<char*>(&bufx[0]),bufx.size()));
    datumx.set_encoded(true);
    datumx.set_label(input.get<int>(1));
    cv::imencode(".jpg", My, bufy);
    datumy.set_channels(1);
    datumy.clear_data();
    datumy.clear_float_data();
    CHECK(My.depth() == CV_8U) << "Image data type must be unsigned byte";
    datumy.set_data(std::string(reinterpret_cast<char*>(&bufy[0]),bufy.size()));
    datumy.set_encoded(true);
    datumy.set_label(input.get<int>(1));
  }
  else {
    ...
    datumx.clear_data();
    datumx.clear_float_data();
    CHECK(M.depth() == CV_8U) << "Image data type must be unsigned byte";
    datumx.set_data(std::string(reinterpret_cast<char*>(&buf[0]),buf.size()));
    datumx.set_encoded(true);
    datumx.set_label(input.get<int>(1));
  }
  output.set(0, datumx.SerializeAsString());
  output.set(1, datumy.SerializeAsString());
}
/*
static void decodeDatum(MEX_ARGS){
InputArguments input(nrhs, prhs, 1);
  OutputArguments output(nlhs, plhs, 1);
  caffe::Datum* datum=&input.get<Datum>(0);
  caffe::DecodeDatum(datum, -1)
  output.set(0, datum.SerializeAsString());
}
*/
static void toDatum(MEX_ARGS){
	 InputArguments input(nrhs, prhs, 2);
	  OutputArguments output(nlhs, plhs, 1);
	  caffe::Datum datum;
	  MxArray array(input.get(0));
