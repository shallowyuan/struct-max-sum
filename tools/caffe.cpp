...
#include "caffe/video_hdf5_layer.hpp"
...
DEFINE_int32(iterations, 1000,
    "The number of iterations to run.");

DEFINE_int32(class,-1,
    "train different detectors for different classes.");
...
int train() {
  ...
  std::vector<shared_ptr<caffe::Layer<float> > > mlayers=(solver->net())->layers();
 	// boost::dynamic_pointer_cast<caffe::VideoEDatumLayer<float> >(mlayers[0])->setclassid(FLAGS_class);
  boost::dynamic_pointer_cast<caffe::LossLayer<float> >(mlayers[mlayers.size()-1])->setclassid(FLAGS_class);
...
int test() {
  ...
  std::vector<shared_ptr<caffe::Layer<float> > > mlayers=caffe_net.layers();
  boost::dynamic_pointer_cast<caffe::OverlapLossLayer<float> >(mlayers[mlayers.size()-1])->setclassid(FLAGS_class);
