...
template <>
void hdf5_load_d_dataset<float>(hid_t file_id, const char* dataset_name_,
        int min_dim, int max_dim, Blob<float>* blob) {
  hdf5_load_d_dataset_helper(file_id, dataset_name_, min_dim, max_dim, blob);
...
template <>
void hdf5_load_d_dataset<double>(hid_t file_id, const char* dataset_name_,
...
template <>
void hdf5_save_d_dataset<float>(
...
template <>
void hdf5_save_d_dataset<double>(
...
string hdf5_load_string(hid_t loc_id, const string& dataset_name) {
  ...
  //dset = H5Dopen (loc_id, dataset_name.c_str(), H5P_DEFAULT);

  status = H5LTread_dataset_string(loc_id, dataset_name.c_str(),buf);
  //status = H5Dread (dset, memtype, H5S_ALL, H5S_ALL, H5P_DEFAULT, (void*)buf);
  //CHECK_GE(H5Tclose (memtype),0);
  CHECK_GE(status, 0)
...
string hdf5_load_string_CPlus(const string& filename, const string& dataset_name) {
  ...
  herr_t status =    H5LTget_dataset_info(file.getId(), dataset_name.c_str(), NULL, &class_, &size);
  CHECK_GE(status, 0) << "Failed to get dataset info for " << dataset_name;
  char *buf = new char[size];
  DataSet dataset = file.openDataSet(dataset_name.c_str());
  DataType dtype = dataset.getDataType();
  //CHECK((native_type=H5Tget_native_type(dtype.getId(), H5T_DIR_DEFAULT))<0) << "H5Tget_native_type  failed!!! ";
  dataset.read((void*)buf, dtype);
  string val=string(buf,size);
  LOG(INFO)<<strlen(buf)<<" "<<dtype.getSize()<<" "<<val.size();
  delete[] buf;
  return val;
}
