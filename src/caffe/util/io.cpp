...
void CVFlowToDatum(const cv::Mat& cv_imgx, const cv::Mat& cv_imgy, Datum* datum) {
  CHECK(cv_imgx.depth() == CV_8U || cv_imgy.depth() == CV_8U ) << "Image data type must be unsigned byte";
  CHECK(cv_imgx.rows == cv_imgx.rows ) << "Two flow images must have the same rows";
  CHECK(cv_imgx.cols == cv_imgx.cols ) << "Two flow images must have the same cols";
  CHECK(cv_imgx.channels()==1);
  CHECK(cv_imgy.channels()==1);
  datum->set_channels(cv_imgx.channels()*2);
  datum->set_height(cv_imgx.rows);
  datum->set_width(cv_imgx.cols);
  datum->clear_data();
  ...
  for (int h = 0; h < datum_height; ++h) {
    const uchar* ptr1 = cv_imgx.ptr<uchar>(h);
    const uchar* ptr2 = cv_imgy.ptr<uchar>(h);
    int img_index = 0;
    for (int w = 0; w < datum_width; ++w) {
        int datum_index = h * datum_width + w;
        buffer[datum_index] = static_cast<char>(ptr1[img_index]);
        datum_index = (datum_height + h) * datum_width + w;
...
bool ReadFlowToDatum(const string& filename, const int label,
    const int height, const int width, const bool is_color,const std::string & encoding, Datum* datumx, Datum* datumy, const int fid) {
  char tmpx[30], tmpy[30];
  sprintf(tmpx,"flow_x_%05d.jpg",fid);
   cv::Mat cv_imgx = ReadImageToCVMat(filename+tmpx, height, width, is_color);
   sprintf(tmpy,"flow_y_%05d.jpg",fid);
   cv::Mat cv_imgy = ReadImageToCVMat(filename+tmpy, height, width, is_color);
  if (cv_imgx.data && cv_imgy.data) {
     if (encoding.size()){
      if ( (cv_imgx.channels() == 3) == is_color && !height && !width &&
          matchExt(tmpx, encoding) )
        return ReadFileToDatum(filename+tmpx, label, datumx) && ReadFileToDatum(filename+tmpy,label,datumy);
      std::vector<uchar> bufx;
      cv::imencode("."+encoding, cv_imgx, bufx);
      datumx->set_data(std::string(reinterpret_cast<char*>(&bufx[0]),
                      bufx.size()));
      datumx->set_label(label);
      datumx->set_encoded(true);
      std::vector<uchar> bufy;
      cv::imencode("."+encoding, cv_imgy, bufy);
      datumy->set_data(std::string(reinterpret_cast<char*>(&bufy[0]),
                      bufy.size()));
      datumy->set_label(label);
      datumy->set_encoded(true);
      return true;
     }
    CVFlowToDatum(cv_imgx, cv_imgy, datumx);
    datumx->set_label(label);
    return true;
  } else {
    return false;
...
cv::Mat DecodeDatumToCVMatNative(const Datum& datum) {
  ...
  cv::resize(cv::imdecode(vec_data, -1),cv_img,cv::Size(320, 240));
  if (!cv_img.data) {
    LOG(ERROR) << vec_data.size()<<"Could not decode datum ";
  }
  return cv_img;
}
