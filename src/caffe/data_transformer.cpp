...
void DataTransformer<Dtype>::Transform(const Datum& datum,
 ...
 //   LOG(INFO)<< " fixed transformation"<<need_imgproc<<"h_off: "<<h_off<<"w_off:"<<w_off<<"crop_width:"<<crop_width<<"crop_height:"<<crop_height;
...
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  if (datum.encoded()) {
    ...
    // If force_color then decode in color otherwise decode in gray.
      ...
      //cv::resize(DecodeDatumToCVMatNative(datum),cv_img,cv::Size(340, 256));
      cv_img = DecodeDatumToCVMatNative(datum);
        //cv::resize(cv_img, cv_img, cv::Size(340, 256));
    }
